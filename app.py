import yaml
from flask import Flask, g, render_template, request, url_for, session, redirect, jsonify
...
def load_categories_and_queries():
    ...
    for i, category in enumerate(categories):
        ...
        for j, query_item in enumerate(category.get('queries', [])):
            ...
            query_item['filters'] = [f"{f[0]} {f[1]} {f[2]}" for f in query_item['query']['filters']]
...
def query():
    if request.method == "POST":
        ...
        # Generate filters
        filters = [get_data_registry().Query.gen_filter(f"{tab}.{col}", op, val) 
                  for tab, col, op, val in zip(tables, columns, operators, values) 
                  if tab and col and val]  # Skip empty conditions
    ...
    if not is_reset:
        previous_query = {
            "tables": session.get("query_tables", []) if isinstance(session.get("query_tables"), list) else [],
            "columns": session.get("query_columns", []) if isinstance(session.get("query_columns"), list) else [],
            "operators": session.get("query_operators", []) if isinstance(session.get("query_operators"), list) else [],
            "values": session.get("query_values", []) if isinstance(session.get("query_values"), list) else [],
            "return_columns": session.get("return_columns", []) if isinstance(session.get("return_columns"), list) else [],
            "schema": session.get("schema", ["lsst_desc_production"]) if isinstance(session.get("schema"), list) else ["lsst_desc_production"]
        }
    else:
...
def run_prebuilt_query():
    ...
    try:
        ...
        if not selected_category or not selected_query:
            raise ValueError(f"Could not find query with title '{query_title}' in category '{category_name}'")
        ...
        filters = [data_registry.Query.gen_filter(f[0], f[1], f[2]) for f in query_filters]
        ...
        session["filters"] = filters
        session["return_columns"] = return_columns
        session["schema"] = ["lsst_desc_production"]  # Default schema for production datasets
...
def clear_query_session():
    ...
    keys_to_clear = [
        "query_tables", "query_columns", "query_operators", "query_values",
        "return_columns", "filters", "reset_state", "query_title", "query_description"
    ]
...
def production_datasets():
    ...
    return render_template("production_datasets.html", categories=categories, error=error)
